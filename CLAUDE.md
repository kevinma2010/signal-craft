...
SignalCraft is an open-source AI intelligence agent that fetches high-signal AI content (builder posts, official blogs, GitHub releases, YouTube, podcasts, X topics), deduplicates and ranks it, and generates evidence-backed daily/weekly briefings. The repository itself is an agent skill targeting multiple runtimes — Claude Code, Codex CLI, and Grok Build all load the same `SKILL.md` natively. Core instructions must never depend on host-specific tools.
